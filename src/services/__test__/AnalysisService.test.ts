...
describe('AnalysisService - top files for every author at once', () => {
  let db: Db
  let service: AnalysisService
  ...
  it('gives each canonical author the same files the one-author query gives, in one call', async () => {
    const all = await service.getTopFilesForAuthors(5)
    const gorvekAlone = await service.getAuthorFileContributions(GORVEK)
    const nightshroudAlone =
      await service.getAuthorFileContributions(NIGHTSHROUD)

    expect(all.get(GORVEK)).toEqual(gorvekAlone)
    expect(all.get(NIGHTSHROUD)).toEqual(nightshroudAlone)
  })

  it('folds an alias into its canonical author rather than listing it apart', async () => {
    const all = await service.getTopFilesForAuthors(5)

    expect(all.has(GORVEK_ALIAS)).toBe(false)
    // b.ts holds 3 lines under the canonical id and 1 under the alias.
    expect(
      all.get(GORVEK)?.find((f) => f.path === 'src/b.ts')?.authorLines,
    ).toBe(4)
  })

  it('keeps only the asked-for number of files per author, the largest first', async () => {
    const all = await service.getTopFilesForAuthors(1)

    expect(all.get(GORVEK)).toHaveLength(1)
    // Gorvek holds 4 lines in both files; the tie is broken by path so that
    // the answer is the same on every run.
    expect(all.get(GORVEK)?.[0]?.path).toBe('src/a.ts')
  })

  it('has no entry for an author who owns no files', async () => {
    const all = await service.getTopFilesForAuthors(5)
